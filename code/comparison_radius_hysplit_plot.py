...
hysplit_pop_exposure.plot(alpha=1, transform=ccrs.PlateCarree())
plt.title('Expected infant deaths per hectare of burned land')
plt.savefig("figures/exp_inf_deaths_no_interpolation.png")
...
plt.title('Expected infant deaths per hectare of burned land')
plt.savefig("figures/exp_inf_deaths_interpolated.png")
...
ax.plot(77.216721, 28.644800, 'bo', markersize=2, transform=ccrs.PlateCarree())
ax.text(77, 28.3, 'New Delhi', transform=ccrs.PlateCarree())
hysplit_pop_exposure_matched_masked.plot(alpha=1, transform=ccrs.PlateCarree())
plt.title('Expected infant deaths per hectare of burned land')
plt.savefig("figures/exp_inf_deaths_interpolated_filtered.png")



bins =np.linspace(start = 0, stop = 0.045, num = 50)
hysplit_pop_exposure_matched.plot.hist(bins = bins, xlim=[0, 0.045], alpha=0.55, density=True)
hysplit_pop_exposure_matched_masked.plot.hist(bins = bins,xlim=[0, 0.045], alpha=0.55, density=True)
plt.legend(['All pixels', 'Only pixels with burned land'], loc="upper left")
plt.xlabel('Expected infant deaths per hectare of burned land')
plt.ylabel('Probablity density')
plt.title('')
plt.savefig("figures/hist_comparison_burned_area_exp_inf_deaths.png")
